        ...
        public override void GenerateSamples()
        {
            ...
            for (int p = 0; p < NumSets; p++)
                for (int i = 0; i < n; i++)
                    for (int j = 0; j < n; j++)
                    {
                        ...
                        samples[s] = SetX(samples[s], (i * n + j) * subcell_width + MathUtil.RandomFloat(0, subcell_width));
                        samples[s] = SetY(samples[s], (j * n + i) * subcell_width + MathUtil.RandomFloat(0, subcell_width));
                    }
            ...
            for (int p = 0; p < NumSets; p++)
                for (int i = 0; i < n; i++)
                    for (int j = 0; j < n; j++)
                    {
                        int k = MathUtil.RandomInt(j, n - 1);
