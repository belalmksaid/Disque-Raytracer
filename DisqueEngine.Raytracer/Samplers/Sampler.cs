    ...
    {             
        ...
        public void ShuffleXCoordinates()
        {
            for (int p = 0; p < NumSets; p++)
                ...
                {
                    int target = MathUtil.RandomInt() % NumSamples + p * NumSamples;
        ...
        public void MapSamplesToDisk()
        {
            ...
            for (int j = 0; j < size; j++)
            {
                ...
                phi *= MathUtil.PI / 4.0f;
                DiskSamples[j] = SetX(DiskSamples[j], r * MathUtil.Cos(phi));
                DiskSamples[j] = SetY(DiskSamples[j], r * MathUtil.Sin(phi));
            }
            samples.Clear();
        }
        ...
        public void MapSamplesToHemisphere(float exp)
        {
            ...
            for (int j = 0; j < size; j++)
            {
                float cos_phi = MathUtil.Cos(2.0f * MathUtil.PI * samples[j].X);
                float sin_phi = MathUtil.Sin(2.0f * MathUtil.PI * samples[j].X);
                float cos_theta = MathUtil.Pow((1.0f - samples[j].Y), 1.0f / (exp + 1.0f));
                float sin_theta = MathUtil.Sqrt(1.0f - cos_theta * cos_theta);
        ...
        public void MapSamplesToSphere()
        {
            ...
            for (int j = 0; j < NumSamples * NumSets; j++)
            {
                ...
                r2 = samples[j].Y;
                z = 1.0f - 2.0f * r1;
                r = MathUtil.Sqrt(1.0f - z * z);
                phi = MathUtil.PI * 2.0f * r2;
                x = r * MathUtil.Cos(phi);
                y = r * MathUtil.Sin(phi);
        ...
        public Vector2 SampleUnitSquare()
        {
            if (count % NumSamples == 0)  									// start of a new pixel
                jump = (MathUtil.RandomInt() % NumSets) * NumSamples;				// random index jump initialised to zero in constructor
        ...
        {
            if (count % NumSamples == 0)  									// start of a new pixel
                jump = (MathUtil.RandomInt() % NumSets) * NumSamples;
        ...
        void random_shuffle(List<int> indices)
        {
            ...
            {
                int temp = indices[i];
                int ind = MathUtil.RandomInt(i + 1);
