...
{
    public class ConeChecker : Texture
    {
        ...
        public override Vector3 GetColor(ShadeRec sr)
        {
            ...
            float phi_size = 2.0f * MathUtil.PI / num_horizontal_checkers;   	// in radians - azimuth angle
            float theta_size = h / num_vertical_checkers;   		// in radians - polar angle
            int i_phi = MathUtil.Floor(phi / phi_size);
            int i_theta = MathUtil.Floor(ph / theta_size);
