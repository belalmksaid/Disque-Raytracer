...
namespace DisqueEngine.Raytracer.Textures.NoiseBased
{
    public class LatticeNoise
    {
        ...
        {
            MathUtil.SetRandomSeed(seed_value);
            for (int i = 0; i < kTableSize; i++)
                value_table[i] = 1.0f - 2.0f * MathUtil.RandomFloat();
        }
		...
		void compute_fbm_bounds()
        {
            if (gain == 1.0f)
                ...
                fbm_max = (1.0f - MathUtil.Pow(gain, num_octaves)) / (1.0f - gain);
            fbm_min = -fbm_max;
        }
        ...
        public virtual float value_turbulence(Vector3 p)
        {
            ...
            for (int j = 0; j < num_octaves; j++)
            {
                turbulence += amplitude * MathUtil.Abs(value_noise(frequency * p));
                amplitude *= 0.5f;
                frequency *= 2.0f;
            }
            turbulence /= fbm_max;
