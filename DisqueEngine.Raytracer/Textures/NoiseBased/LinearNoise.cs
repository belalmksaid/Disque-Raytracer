...
namespace DisqueEngine.Raytracer.Textures.NoiseBased
{
    public class LinearNoise : LatticeNoise
    {
        ...
        public override float value_noise(Vector3 p)
        {
            ...
            ix = MathUtil.Floor(p.X);
            fx = p.X - ix;
            iy = MathUtil.Floor(p.Y);
            fy = p.Y - iy;
            iz = MathUtil.Floor(p.Z);
            fz = p.Z - iz;
            for (int k = 0; k <= 1; k++)
                for (int j = 0; j <= 1; j++)
                    for (int i = 0; i <= 1; i++)
            ...
            x0 = MathUtil.Lerp(fx, d[0, 0, 0], d[0, 0, 1]);
            x1 = MathUtil.Lerp(fx, d[0, 1, 0], d[0, 1, 1]);
            x2 = MathUtil.Lerp(fx, d[1, 0, 0], d[1, 0, 1]);
            x3 = MathUtil.Lerp(fx, d[1, 1, 0], d[1, 1, 1]);
            y0 = MathUtil.Lerp(fy, x0, x1);
            y1 = MathUtil.Lerp(fy, x2, x3);
            z0 = MathUtil.Lerp(fz, y0, y1);
            return (z0);
        }
