...
namespace DisqueEngine.Raytracer.Cameras
{
    public class Spherical : Camera
    {
        ...
        Vector3 ray_direction(Vector2 pp, int hres, int vres, float s)
        {
            ...
            float lambda = pn.X * LambdaMax * MathUtil.PIOn180;
            float psi = pn.Y * PSIMax * MathUtil.PIOn180;
            float phi = MathUtil.PI - lambda;
            float theta = 0.5f * MathUtil.PI - psi;
            float sin_phi = MathUtil.Sin(phi);
            float cos_phi = MathUtil.Cos(phi);
            float sin_theta = MathUtil.Sin(theta);
            float cos_theta = MathUtil.Cos(theta);
