...
namespace DisqueEngine.Raytracer
{
    class MathUtil
    {
        ...
        public const float BigEpsilon = 0.001f;
        ...
        {
            float costheta = MathUtil.Lerp(u1, costhetamax, 1.0f);
            float sintheta = MathUtil.Sqrt(1.0f - costheta * costheta);
            float phi = u2 * 2.0f * MathUtil.PI;
            return MathUtil.Cos(phi) * sintheta * x + MathUtil.Sin(phi) * sintheta * y + costheta * z;
        }
        public static void ConcentricSampleDisk(float u1, float u2, ref float dx, ref float dy)
        {
            ...
            theta *= MathUtil.PI / 4.0f;
            dx = r * MathUtil.Cos(theta);
            dy = r * MathUtil.Sin(theta);
        }
