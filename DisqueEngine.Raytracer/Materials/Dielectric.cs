...
using System.Text;
using DisqueEngine.Raytracer.BTDFs;
using DisqueEngine.Raytracer.BRDFs;
using DisqueEngine.Math;
using DisqueEngine.Raytracer.Samplers;

namespace DisqueEngine.Raytracer.Materials
{
    public class Dielectric : Material
    {
        ...
        public override Vector3 Shade(ShadeRec sr)
        {
            ...
            else
            {
                ...
                if (ndotwi < 0.0)
                {
                    Lr = fr * sr.World.Tracer.TraceRay(reflected_ray, ref t, sr.Depth + 1) * MathUtil.Abs(ndotwi);
                    L += cf_in.Pow(t) * Lr;
                    Lt = ft * sr.World.Tracer.TraceRay(transmitted_ray, ref t, sr.Depth + 1) * MathUtil.Abs(ndotwt);
                    L += cf_out.Pow(t) * Lt;
                }
                else
                {
