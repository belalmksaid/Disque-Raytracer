...
namespace DisqueEngine.Raytracer.GeometricObjects.PartObjects
{
    public class PartAnnulus : GeometricObject
    {
            ...
            : base(name)
        {
            ...
            min_azimuth = MathUtil.ToDegrees(m_azimuth);
            max_azimuth = MathUtil.ToDegrees(mx_azimuth);
            bbox = new BBox(center.X - inner_radius - wall_thickness, center.X + inner_radius + wall_thickness, center.Y - MathUtil.Epsilon, center.Y + MathUtil.Epsilon, center.Z - inner_radius - wall_thickness, center.Z + inner_radius + wall_thickness);
        ...
        public override bool Hit(Ray ray, ref float tmin, ref ShadeRec sr)
        {
            ...
            if (v < w_squared && v > i_squared)
            {
                ...
                double phi =MathUtil.Atan2(p.X, p.Z);
                if (phi < 0.0)
                    phi += MathUtil.TwoPI;
